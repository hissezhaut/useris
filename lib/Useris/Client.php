<?php
	...
	use \Useris\Exceptions\UserisException;
	use \Useris\Exceptions\ServiceException;
	use \Useris\Exceptions\TransportException;
	use \Useris\Exceptions\InvalidConfigurationException;
	...
	class Client extends ClientBase {
		...
		public function call($version, $service, $method, $arguments){
			...
			$headers = array(
				...
				'User-Agent' => sprintf("Useris/%s PHP/%s", self::CLIENT_VERSION, PHP_VERSION),  
			...
			if($has_error){
				...
				if($error_code == 'invalid_credentials' || $error_code == 'unauthorized'){
					$this->emit('unauthorized', array($this, $call_context, &$result));
				}

				$this->emit('error', array($this, $call_context, &$result));
			}else{
				$this->emit('success', array($this, $call_context, &$result));
			}
